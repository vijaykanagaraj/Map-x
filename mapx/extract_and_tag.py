    ...
    def extract(self):
        ...
        for index, contour in enumerate(self.contours):
            ...
            if content:
                if type == "table":
                    self.basic_info.content_holder.table_contours.append(contour)
                    if self.basic_info.extraction_filter.kind in ("TABLE", "ALL"):
                        key = "-".join(("T", f"{int(self.basic_info.pdf_property.page_no) + 1}#{table_count}", f"{center[0]}#{center[1]}"))
                    ...
                    if self.basic_info.extraction_filter.kind in ("PARAGRAPH", "ALL"):
                        key = "-".join(("P", f"{int(self.basic_info.pdf_property.page_no) + 1}#{paragraph_count}", f"{center[0]}#{center[1]}"))
